-- RENFLIX: Push Payment no longer asks for a month. The request is dated to today,
-- while the tenant notification remains month-neutral.
DROP FUNCTION IF EXISTS public.push_payment_request(UUID, UUID, NUMERIC, DATE, TEXT);
...
BEGIN
  ...
  VALUES (
    ...
    p_amount, CURRENT_DATE, 'PENDING',
    CASE
      WHEN p_remarks IS NULL OR btrim(p_remarks) = '' THEN 'Payment request'
      ELSE format('Payment request. Remarks: %s', btrim(p_remarks))
    END
  )
  ...
  IF v_tenant.profile_id IS NOT NULL THEN
    ...
    VALUES (
      ...
      format(
        '%s has requested ₹%s. Open Payments to pay now.',
...
REVOKE ALL ON FUNCTION public.push_payment_request(UUID, UUID, NUMERIC, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.push_payment_request(UUID, UUID, NUMERIC, TEXT) TO authenticated;
