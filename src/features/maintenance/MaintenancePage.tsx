...
export default function MaintenancePage() {
  ...
  async function fetchAll() {
    ...
    try {
      if (isTenant && user) {
        ...
        const { data: tenantRequests, error: requestError } =
          await supabase
        ...
        setRequests(
          (tenantRequests || []) as MaintenanceRequest[]
        );
      } else {
        ...
        setRequests(
          (reqRes.data || []) as MaintenanceRequest[]
        );

        setProperties(
          (propRes.data || []) as Property[]
        );
      }
    } catch (error) {
      setToast({
  ...
  async function handleTenantSubmit(e: React.FormEvent) {
    ...
    if (!profile?.organization_id || !tenant || !unit) {
      setToast({
        msg: "Your tenant account is not linked to a unit.",
        type: "error",
      });
      return;
    }
    ...
    if (error) {
      ...
      setToast({
        msg: "Maintenance request submitted to the owner.",
        type: "success",
      });
  ...
  async function deleteSelectedMaintenance() {
    if (!selectedIds.length) {
      setToast({
        msg: "Select at least one maintenance request.",
        type: "error",
      });
      return;
    }
    ...
    if (
      !(await appConfirm(
        `Archive ${selectedIds.length} selected maintenance request(s)? They will remain in Settings → Archived.`
      ))
    ) {
      return;
    }
    ...
    setToast({
      msg: "Selected maintenance requests archived.",
      type: "success",
    });
            ...
            <div className="flex items-center gap-2">
              {selectionMode ? (
                <Button
                  variant="secondary"
                  size="sm"
                  onClick={deleteSelectedMaintenance}
                ...
                <div className="flex items-start gap-3">
                  {selectionMode && (
                    <button
                      type="button"
                      onClick={() =>
                        toggleMaintenanceSelection(req.id)
                      }
                    ...
                    >
                      ...
                      >
                        {selectedIds.includes(req.id) && (
                          <span className="text-white text-[10px] leading-none m-auto">
                            ✓
                          </span>
                        )}
                      </span>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {/* FIX: Status pill now appears for tenants */}
  ...
  /*
   * =========================================================
   * OWNER / PROPERTY MANAGER MAINTENANCE PAGE
   * =========================================================
   */
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Maintenance"
        subtitle={`${openCount} open request${
          openCount !== 1 ? "s" : ""
        }`}
        action={
          selectionMode ? (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-[1fr_180px_180px] gap-3 mb-5">
        ...
        <select
          className="w-full bg-navy-800 border border-navy-700 rounded-lg px-3 py-2 text-sm text-navy-100"
          value={filterStatus}
          onChange={(e) =>
              ...
              <Select
                ...
                onChange={async (e) => {
                  ...
                  if (error) {
                    ...
                    setToast({
                      msg: `Status updated to ${nextStatus.replace(
                        /_/g,
                        " "
                      )}.`,
                      type: "success",
