...
export default function MessagesPage() {
  ...
  // On small screens, if no active conversation → show list; if active → show chat only
  const showSidebar = !activeConv; // We'll use CSS to handle it
  ...
  async function createConversation() {
    ...
    const { data, error } = await supabase
      .from("conversations")
      .insert({ organization_id: profile!.organization_id!, title: newTitle })
      .select()
      .single();
    if (error) {
  ...
  return (
    <div className="animate-fade-in h-[calc(100vh-120px)]">
      ...
      {/* Main chat area — responsive grid */}
      <div className="flex gap-4 h-[calc(100%-64px)]">
        {/* Sidebar (hidden on mobile when a chat is active) */}
        <div
          className={`w-full md:w-72 flex-shrink-0 bg-navy-800 border border-navy-700 rounded-xl overflow-y-auto
            ...
            <div className="flex flex-col">
              {conversations.map((conv) => (
                <button
                  ...
                  className={`text-left px-4 py-3 border-b border-navy-700 hover:bg-navy-700/40 transition-colors
                    ${activeConv?.id === conv.id ? "bg-blue-600/15 border-l-2 border-l-blue-500" : ""}`}
                >
        ...
        {/* Chat panel (visible on mobile when activeConv is set) */}
        <div
          className={`flex-1 bg-navy-800 border border-navy-700 rounded-xl flex flex-col overflow-hidden
          ...
          {!activeConv ? (
            ...
            <>
              ...
              <div className="px-4 py-3 border-b border-navy-700 flex-shrink-0 flex items-center gap-2">
                <button
                  className="md:hidden text-navy-400 hover:text-white p-1 -ml-1"
                  ...
                  return (
                    <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                      ...
                      >
                        <p>{msg.content}</p>
                        <div
                          className={`text-[10px] mt-1 ${isMe ? "text-blue-200" : "text-navy-500"}`}
                        >
